#!/usr/bin/env python3
"""Test vLLM with 15K context length for K2-Thinking.

Verifies that vLLM can handle 15,360 tokens input (matching Megatron training max).
"""
...
CONTEXT_LENGTH = 15360  # Match Megatron training max
...
def create_sampling_session():
    ...
    resp = requests.post(
        ...
        json={"base_model": MODEL, "session_id": f"vllm_15k_test_{int(time.time())}"},
    ...
    if "request_id" in data:
        ...
        result = poll_future(data["request_id"], timeout=300)
        if "error" in result:
...
def main():
    ...
    # Test various context lengths up to 15K
    test_lengths = [1024, 4096, 8192, 12288, 15360]
