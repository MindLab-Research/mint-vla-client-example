...
Requires:
1. Megatron trained and PEFT exported to vLLM
2. vLLM actor running with the new compute_prompt_topk method

Run locally: python scripts/compare_vllm_megatron_topk.py
"""
...
def get_megatron_topk_from_diag():
    """Parse Megatron diagnostic log for top-K info at position 7."""
    log = run_ssh('tail -100 /vePFS-Mindverse/share/code/raw_logit_diag.log 2>/dev/null | grep "pos=7"')
    return log
...
async def main():
    ...
    # === Get vLLM top-K ===
    print("\n" + "=" * 60)
    print("Getting vLLM top-K at position 7...")
    print("=" * 60)
    ...
    try:
        topk_result = ray.get(vllm_actor.compute_prompt_topk.remote(
            ...
            request_id="topk_compare_001",
            k=10,
        ), timeout=120)

        print(f"\nvLLM returned {len(topk_result)} positions")

        if len(topk_result) > 7:
            pos7_topk = topk_result[7]
            print(f"\nvLLM Top-K at position 7 (predicting token 8):")
            print(f"  Target token: {target_tokens[7]} ('{tokenizer.decode([target_tokens[7]])}')")
            print()
            ...
            print(f"ERROR: Position 7 not available (only got {len(topk_result)} positions)")
    ...
    # === Get Megatron diagnostic ===
    print("\n" + "=" * 60)
    print("Getting Megatron diagnostic from log...")
    print("=" * 60)

    megatron_diag = get_megatron_topk_from_diag()
    if megatron_diag:
        print(megatron_diag)
    else:
        print("No Megatron diagnostic found in log")
        print("(Run a forward pass through Megatron to generate diagnostic)")
