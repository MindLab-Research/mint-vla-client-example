...
if [[ "${ISSUE_STARTUP_PRINT_ENV:-0}" == "1" ]]; then
  env | grep -E '^(RAY_ADDRESS|MINT_RAY_CLIENT_ADDRESS|MINT_CODE_ROOT|MINT_RAY_PY_MODULES_CSV|MINT_VLLM_CHILD_PYTHON_EXECUTABLE|MINT_RAY_NAMESPACE|MINT_MODEL_WORK_SCHEDULER_ACTOR_NAME|MINT_TASK_STATE_STORE_ACTOR_NAME|MINT_MAINTENANCE_CRON_ACTOR_NAME|MINT_DISABLE_MINT_ROUTE|MINT_API_KEY|MINT_PORT|MINT_LOG_FILE|MINT_USAGE_LOG_DIR|MINT_MODEL_PLACEMENT_JSON|MINT_DENSE_MODEL_PLACEMENT_JSON|MINT_VLLM_MODEL_PLACEMENT_JSON|MINT_MEGATRON_MODEL_PLACEMENT_JSON)=' | sort
  exit 0
fi
