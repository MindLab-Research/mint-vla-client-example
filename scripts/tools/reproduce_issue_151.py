...
def _require_model_in_caps(model_name: str) -> None:
    ...
    for m in models:
        if isinstance(m, dict) and m.get("model_name") == model_name:
            return
    raise RuntimeError(f"model {model_name!r} not present in supported_models (wrong server or wrong gateway config)")
...
def main() -> int:
    ...
    try:
        ...
        session_id = _post_json("/api/v1/create_session", {"tags": ["issue-151"]}, timeout_s=30.0).get(
            "session_id"
        )
        ...
        # Bug #151: on gateway deployments, /save_state returned 404 for routed models because
        # weights routes did not consult remote_training_model() and did not forward upstream.
        save_state = _post_json(
            "/api/v1/save_state",
            {"model_id": model_id, "path": f"issue-151-{int(time.time())}"},
            timeout_s=30.0,
        )
