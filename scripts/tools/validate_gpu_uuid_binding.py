...
def _read_resource_pool_bindings() -> list[dict[str, str]]:
    try:
    ...
    for rec in stats.get("resource_pool", []):
        if not isinstance(rec, dict):
            continue
        for binding in internal_routes._resource_pool_gpu_bindings(rec):
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fetch current ResourcePool bindings and verify any gpu_uuid labels against NVML UUIDs on the same hostname.",
    )
    ...
    resource_pool_bindings = _read_resource_pool_bindings() if args.validate_existing_bindings else []
    resource_pool_binding_uuid_errors = []
    for binding in resource_pool_bindings:
        ...
        if gpu_uuid and hostname and gpu_uuid not in nvml_by_hostname.get(hostname, set()):
            resource_pool_binding_uuid_errors.append(binding)
    summary = []
    for row in rows:
        ...
        json.dumps(
            {
                ...
                "resource_pool_binding_uuid_count": sum(
                    1 for binding in resource_pool_bindings if binding.get("gpu_uuid")
                ),
                "resource_pool_binding_uuid_missing_from_nvml": resource_pool_binding_uuid_errors,
            },
            indent=2,
            sort_keys=True,
        )
