...
FORWARD_MODEL = (
    os.environ.get("TINKER_FORWARD_MODEL")
    or os.environ.get("TINKER_MODEL")
...
def _poll_future(request_id: str) -> dict[str, Any]:
    ...
    while time.time() < deadline:
        ...
        if resp.status_code == 408:
            ...
            continue
        raise RuntimeError(f"POST {url} returned {resp.status_code}: {resp.text[:200]!r}")
    raise RuntimeError(f"retrieve_future timed out after {POLL_TIMEOUT_S}s (request_id={request_id})")


def main() -> int:
    try:
        caps = _get_json(f"{BASE_URL}/api/v1/get_server_capabilities", timeout_s=30.0)
        _require_model_in_caps(caps, FORWARD_MODEL)

        session = _post_json(
            f"{BASE_URL}/api/v1/create_session",
            {
                "tags": ["scripts/tools/reproduce_issue_105.py", f"repro-105-{uuid.uuid4().hex[:8]}"],
                "user_metadata": {},
                "sdk_version": "scripts/tools/reproduce_issue_105.py",
            },
            timeout_s=30.0,
        )
        ...
        if "error" in result:
            return _fail(f"retrieve_future returned error: {result.get('error')!r}")
        sequences = result.get("sequences")
        if not isinstance(sequences, list) or not sequences:
            return _fail(f"retrieve_future missing sequences: {result!r}")
