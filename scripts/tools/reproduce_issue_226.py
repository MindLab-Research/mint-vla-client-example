...
def _ssh_create_named_actor(actor: _NamedActor, *, block_s: int | None = None) -> None:
    # Create a detached named actor in the server namespace, without registering it in ResourcePool.
...
def main() -> int:
    ...
    # ResourcePool has no VLLM entries. Before the fix, the endpoint kills the
    ...
    try:
        ...
        _require(
            ...
            f"precondition_failed: {named_only.name} unexpectedly appears in ResourcePool",
        )
