...
def _create_model(base_url: str, headers: dict[str, str], *, model: str, idx: int) -> str:
    payload = {
        "session_id": f"pressure-{idx}-{uuid.uuid4().hex[:8]}",
        "model_seq_id": 0,
        "base_model": model,
        "lora_config": {"rank": 16, "train_attn": True, "train_mlp": True, "train_unembed": True},
        "user_metadata": {"script": "scripts/wip/openpi_vla_pressure_train_threads.py", "worker": idx},
    }
...
def _train_once(base_url: str, headers: dict[str, str], *, model_id: str) -> dict[str, Any]:
    payload = {"model_id": model_id, "loss_fn": "cross_entropy", "data": [_datum()]}
...
def _worker(base_url: str, headers: dict[str, str], *, idx: int, model: str, steps: int, start_event: threading.Event) -> dict[str, Any]:
    start_event.wait()
    model_id = ""
    losses: list[float] = []
    try:
        model_id = _create_model(base_url, headers, model=model, idx=idx)
        for _ in range(steps):
            result = _train_once(base_url, headers, model_id=model_id)
            losses.append(float(result.get("metrics", {}).get("loss:mean", 0.0)))
        return {"worker": idx, "ok": True, "model_id": model_id, "steps": steps, "losses": losses}
    except Exception as exc:
        return {"worker": idx, "ok": False, "error": repr(exc)}
    finally:
        if model_id:
...
def main() -> int:
    ...
    parser.add_argument("--steps-per-worker", type=int, default=2)
    ...
    summary = {
        ...
        "results": sorted(results, key=lambda r: int(r["worker"])),
    }
    if args.output_json:
