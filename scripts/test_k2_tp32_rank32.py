#!/usr/bin/env python3
"""Test K2 with TP=32, EP=96, ETP=1, rank=32 for 30K context.

Constraint: rank must be divisible by TP for attention/shared_expert layers.
32 % 32 == 0 ✓
"""
import os
import sys
...
def main():
    print("=" * 60)
    print("Testing K2: TP=32, EP=96, ETP=1, rank=32 for 30K context")
    print("=" * 60)
    print("Config: 96 GPUs, 4 experts/GPU")
    print("Parallelism: TP=32 for attention/shared_expert, ETP=1 for routed experts")
    print("Constraint: rank=32 % TP=32 == 0 (satisfied)")
    print()

    # Kill existing Megatron actor
    ...
    # Create model with rank=32
    sid = f"k2_tp32_rank32_{uuid.uuid4().hex[:8]}"
    print(f"\nCreating model with rank=32...")
    r = requests.post(
    ...
    print("\n" + "=" * 60)
    print(f"Max working context with TP=32, EP=96, rank=32: {max_working}")
    print("=" * 60)
