#!/usr/bin/env python3
"""Get vLLM top-K with trained LoRA using correct lora_int_id.

Run on volcano after training and export:
python3 /root/tinker_project/tinker-server/scripts/get_trained_vllm_topk.py
"""
...
async def main():
    ...
    # The LoRA ID is typically 1 for the first loaded adapter
    # (multi-LoRA engine allocates sequential IDs starting from 1)
    lora_id = 1
    print(f"Using lora_id={lora_id}")

    # Call compute_prompt_topk_with_lora
    print("\n" + "=" * 60)
    print("vLLM Top-K at position 7 (with trained LoRA)")
    print("=" * 60)

    try:
        topk_result = ray.get(vllm_actor.compute_prompt_topk_with_lora.remote(
            prompt_ids=input_tokens,
            request_id="topk_trained_001",
            lora_int_id=lora_id,
            k=10,
        ), timeout=120)
