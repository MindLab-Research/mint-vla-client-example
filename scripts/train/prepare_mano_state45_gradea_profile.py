#!/usr/bin/env python3
"""Publish the phase-aware State45 profile over the immutable Grade-A release."""
...
from scripts.train.state41_gradea_contract import canonical_sha256
...
PROFILE_CONTRACT = "mano_state45_grade_a_train_profile_v1"
...
def grade_a_records(light_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for release_row_index, source in enumerate(light_rows):
        ...
        record["prompt"] = canonical_release_full_task_prompt(record)
...
def _normalized_value_for_discrete_bin(bin_id: int) -> np.float32:
    """Return a normalized scalar that Pi0.5 maps to exactly *bin_id*."""
    if bin_id == -1:
        return np.float32(-2.0)
    if bin_id == 256:
        return np.float32(2.0)
    if not 0 <= bin_id <= 255:
        raise ValueError(f"Pi0.5 discrete bin must be in [-1,256], got {bin_id}")
    return np.float32(-1.0 + 2.0 * bin_id / 256.0)


def _counterfactual_token_audit(
    *,
    object_names: Sequence[str],
    max_tokens: int,
    gesture_ids: Sequence[int] = tuple(range(100)),
    bin_ids: Sequence[int] = tuple(range(-1, 257)),
) -> dict[str, Any]:
    """Audit every repeated legal discrete bin under all formal prompt shapes.

    Repeating each bin across all 45 coordinates measures the longest per-value
    SentencePiece representation, including the two out-of-range bins reachable
    when normalized online state falls outside the train q01/q99 interval.
    """
    tokenizer = PaligemmaTokenizer(
        max_len=max_tokens, fail_on_truncation=True
    )
    maximum = -1
    overflow_count = 0
    historical_200_overflow_count = 0
    maximum_examples: list[dict[str, Any]] = []
    samples = 0
    for object_name in sorted(set(map(str, object_names))):
        for gesture_id in gesture_ids:
            if not 0 <= int(gesture_id) <= 99:
                raise ValueError(f"gesture id must be in [0,99], got {gesture_id}")
            prompt = FULL_TASK_PROMPT_TEMPLATE.format(
                object=object_name, gesture=f"{int(gesture_id):02d}"
            )
            for bin_id in bin_ids:
                normalized = np.full(
                    STATE_DIM,
                    _normalized_value_for_discrete_bin(int(bin_id)),
                    dtype=np.float32,
                )
                length = raw_token_length(tokenizer, prompt, normalized)
                samples += 1
                overflow_count += int(length > max_tokens)
                historical_200_overflow_count += int(length > 200)
                example = {
                    "object": object_name,
                    "gesture": f"{int(gesture_id):02d}",
                    "discrete_bin": int(bin_id),
                    "raw_tokens": int(length),
                }
                if length > maximum:
                    maximum = int(length)
                    maximum_examples = [example]
                elif length == maximum and len(maximum_examples) < 20:
                    maximum_examples.append(example)
    if not samples:
        raise RuntimeError("State45 counterfactual token audit has no samples")
    if overflow_count:
        raise RuntimeError(
            f"State45 counterfactual audit found {overflow_count}/{samples} "
            f"samples above max_token_len={max_tokens}"
        )
    return {
        "samples": samples,
        "discrete_bin_range": [-1, 256],
        "gesture_range": ["00", "99"],
        "max": maximum,
        "overflow_count": overflow_count,
        "historical_200_overflow_count": historical_200_overflow_count,
        "fits_historical_200": historical_200_overflow_count == 0,
...
def _token_audit(
    *,
    ...
    for local_row in sorted(dataset._row_start_offset):
        ...
        expected = canonical_release_full_task_prompt(row["index"])
        if prompt != expected:
            raise ValueError(
                f"canonical prompt mismatch at release row {source_row}: {prompt!r} != {expected!r}"
            )
        sequence = np.asarray(
            dataset._get_state45_sequence(local_row), dtype=np.float32
        )
    ...
    if overflow_count:
        raise RuntimeError(
            f"State45 Grade-A token audit found {overflow_count}/{len(lengths)} "
            f"samples above max_token_len={max_tokens}"
        )
    return {
        "samples": len(lengths),
...
def main() -> int:
    ...
    try:
        ...
        if np.asarray(norm_stats["state"].mean).shape != (STATE_DIM,):
            raise RuntimeError("computed state normalization width is not 45")
        ...
        counterfactual_token_audit = _counterfactual_token_audit(
            object_names=[row["object"] for row in population],
            max_tokens=profile.max_tokens,
        )
        final = output_dir
        report = {
            ...
            "language_conditioning": GESTURE_LANGUAGE,
            "prompt_template": FULL_TASK_PROMPT_TEMPLATE,
            "model": ACTION_LORA_R16_STATE45_MODEL,
            "profile_id": profile.profile_id,
            "source_state_contract": STATE41_CONTRACT_ID,
            ...
            "counterfactual_token_audit": counterfactual_token_audit,
            "sampling_default": {
            ...
            "implementation_sha256": {
                ...
                "split_contract": sha256(
                    Path(__file__).with_name("state45_gradea_contract.py")
                ),
            },
        }
