#!/usr/bin/env python3
"""State56 closed-loop adapter over the authoritative ManoRL native28 model.
...
def _runtime() -> dict[str, Any]:
    ...
    if expected != state56.EXPECTED_MANORL_COMMIT:
        raise RuntimeError("MANORL_EXPECTED_COMMIT must equal the State56 pinned commit")
...
def runtime_provenance(object_name: str | None = None) -> dict[str, Any]:
    ...
    if all_assets_commit != state56.EXPECTED_ALL_ASSETS_COMMIT:
        raise RuntimeError("MANORL_ALL_ASSETS_COMMIT must equal the State56 pinned asset commit")
    result = {
        "repository": str(root),
    ...
    if object_name is not None:
        ...
        if result["curated_asset_source_commit"] != state56.EXPECTED_CURATED_ASSET_SOURCE_COMMIT:
            raise RuntimeError("State56 curated asset source commit mismatch")
    return result
...
def _legacy_visual_model(
    ...
    head_camera_presets = {
        ...
        "legacy": dict(legacy.HEAD_CAMERA),
    }
    if head_camera_preset is None:
        head_camera_preset = "current"
    try:
        head_camera = dict(head_camera_presets[head_camera_preset])
    except KeyError as exc:
        raise ValueError(f"unsupported State56 head camera preset {head_camera_preset!r}") from exc
...
def state56_features_from_mujoco(
    model: Any, data: Any, object_name: str,
) -> tuple[np.ndarray, np.ndarray, list[dict[str, Any]]]:
    """Return State56 contact5, log1p summed normal force5, and raw pairs."""
    import mujoco
    from scripts import mano_state56_contract as contract

    hand_geoms, object_geoms, _floor_geom = _contact_ids(model, object_name)
    ...
    for contact_index in range(int(data.ncon)):
        ...
        if first in hand_geoms and second in object_geoms:
            hand_geom, object_geom, normal_sign = first, second, 1.0
        elif second in hand_geoms and first in object_geoms:
            hand_geom, object_geom, normal_sign = second, first, -1.0
        else:
            continue
        body_id = int(model.geom_bodyid[hand_geom])
        body_name = mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_BODY, body_id) or ""
        finger = next((name for name in contract.FINGER_NAMES if body_name.startswith(name)), None)
        force = np.zeros(6, dtype=np.float64)
        mujoco.mj_contactForce(model, data, contact_index, force)
        magnitude = abs(float(force[0]))
        normal = normal_sign * np.asarray(contact.frame[:3], dtype=np.float64)
        if finger is not None:
            finger_index = contract.FINGER_NAMES.index(finger)
            contacts[finger_index] = 1.0
            loads[finger_index] += magnitude
        records.append({
            "finger": finger or "palm",
            "hand_geom": mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_GEOM, hand_geom) or str(hand_geom),
            "object_geom": mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_GEOM, object_geom) or str(object_geom),
            "position_world": np.asarray(contact.pos, dtype=np.float32).tolist(),
            "normal_force_world": (normal * magnitude).astype(np.float32).tolist(),
            "normal_force_norm": np.float32(magnitude).item(),
            "signed_distance_m": np.float32(contact.dist).item(),
        })
    force_log1p = np.log1p(loads).astype(np.float32)
    if not np.all(np.isfinite(force_log1p)):
        raise FloatingPointError("State56 live contact force is non-finite")
    return contacts, force_log1p, records
