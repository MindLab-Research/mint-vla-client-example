...
class _ModelActorInventorySection(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class MintConfigFile(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
def load_mint_config_file(path: str | Path) -> MintConfigFile:
    p = Path(path)
    ...
    try:
        return MintConfigFile.model_validate(data)
