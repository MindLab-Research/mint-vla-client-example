...
async def cleanup_stale_actors_once() -> dict[str, int]:
    ...
    def _strip_prefixed_name(name: str, prefixes: tuple[str, ...]) -> str | None:
    ...
    mint_actors = [a for a in actors if a.get("namespace") == PERSISTENT_NAMESPACE]
    if not mint_actors:
        logger.info("No actors found in namespace %s", PERSISTENT_NAMESPACE)
        return {"cleaned": 0, "registered": 0}

    logger.info("Found %s actors in namespace %s, checking status...", len(mint_actors), PERSISTENT_NAMESPACE)
    ...
    for actor_info in mint_actors:
        ...
        try:
            ...
            try:
                ...
                metadata: dict[str, object] = {}
                vllm_model_part = _strip_prefixed_name(name, ("mint_vllm_",))
                megatron_model_part = _strip_prefixed_name(name, ("mint_megatron_",))
                if vllm_model_part is not None:
                    actor_type = ActorType.VLLM
                    base_model = ""
                    num_gpus: int | None = None
                    model_name, cfg = _lookup_model_config(vllm_model_part)
                    if cfg is not None:
                    ...
                    if num_gpus is None:
                        ...
                        continue
                elif name.startswith("mint_dense_"):
                    actor_type = ActorType.DENSE
                    num_gpus = 1
                    base_model = ""
                elif megatron_model_part is not None:
                    actor_type = ActorType.MEGATRON
                    base_model = ""
                    num_gpus: int | None = None
                    model_name, cfg = _lookup_model_config(megatron_model_part)
                    if cfg is not None:
                        base_model = model_name
                ...
                from mint_server.backend.model_registry import is_persistent_model
