...
def _remove_actor_pg(actor_name: str, *, namespace: str | None = None) -> None:
    from mint_server.backend.ray_cluster.ray_placement_groups import PlacementGroupNotFoundError, get_named_placement_group
    import ray
...
async def _kill_exact_vllm_actor(*, actor_name: str) -> int:
    ...
    from mint_server.backend.actors.model_actor_supervisor import ActorType, ModelActorSupervisorStaleError, get_model_actor_supervisor
...
async def _kill_exact_megatron_actor(*, actor_name: str) -> int:
    from mint_server.backend.training.megatron.megatron_distributed import PERSISTENT_NAMESPACE
    from mint_server.backend.actors.model_actor_supervisor import ActorType, get_model_actor_supervisor
...
async def kill_actors(request: Request, body: KillActorsRequest) -> dict[str, Any]:
    ...
    if t in ("vllm", "all"):
        from mint_server.backend.inference.multi_lora_engine import kill_persistent_vllm_actor
        from mint_server.backend.actors.model_actor_supervisor import ModelActorSupervisorStaleError
    ...
    if t in ("megatron", "all"):
        from mint_server.backend.training.megatron.megatron_distributed import kill_megatron_actor
