...
def _create_extended_server_class(
    ...
    class ExtendedVLLMHttpServer(_BaseVLLMHttpServer):
        ...
        async def add_lora(self, lora_request) -> None:
            ...
            """
            from mint_server.backend.inference.lora_utils import maybe_validate_peft_adapter_checkpoint_shapes
...
'''
        ...
        async def test_actual_worker_spawn(self) -> dict:
            ...
            import os
            from vllm.utils.system_utils import get_mp_context
            from mint_server.spawn_worker_test import spawn_worker_check, RESULT_FILE
...
class VerlInferenceEngine:
    ...
    async def initialize(self) -> None:
        ...
        if not ray.is_initialized():
            address = strict_ray_gcs_address()
            if not address:
