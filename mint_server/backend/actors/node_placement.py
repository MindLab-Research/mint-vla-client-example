...
class GpuNode:
    ...
    provider_resource_queue_id: str | None
...
def _parse_provider_job_id_from_hostname(hostname: str | None) -> str | None:
...
def _parse_provider_worker_index_from_hostname(hostname: str | None) -> int | None:
    # Volcano replicas show up as: t-<job_id>-worker-<idx>
    if not hostname:
        return None
    ...
    for n in ray.nodes():
        ...
        nodes.append(
            GpuNode(
                ...
                provider_job_id=_parse_provider_job_id_from_hostname(hostname),
...
def _parse_model_gpu_placement_value(
    *,
    ...
    if isinstance(value, dict) and "slices" in value:
        ...
        and ("node_ip" in value or "node" in value or "ip" in value)
    ...
    for idx, item in enumerate(value):
        ...
        if selected_replica is not None and item_replica != selected_replica:
            continue
        if item.get("worker_index") is not None or item.get("worker") is not None or item.get("worker_idx") is not None:
            raise RuntimeError(
                f"{context}: {env_var_name}[{selected_key!r}][{idx}] uses worker_index; use node_ip"
            )
        raw_node_ip = item.get("node_ip", item.get("node", item.get("ip")))
        node_ip = str(raw_node_ip).strip() if raw_node_ip is not None else None
        if not node_ip:
            raise RuntimeError(
                f"{context}: {env_var_name}[{selected_key!r}][{idx}] requires node_ip"
            )
        try:
        ...
        slices.append((item_replica, None, gpu_count, node_ip))
    ...
    *,
    ...
    for raw in worker_gpu_slices:
        if len(raw) == 3:
            ...
            raise RuntimeError(
                f"{context}: worker_index placement is no longer supported; use node_ip "
                f"(replica={replica}, worker_index={worker_index}, gpu_count={gpu_count})"
            )
        else:
    ...
    for replica, worker_idx, gpu_count, node_ip in normalized_slices:
        ...
        worker_idx = _parse_provider_worker_index_from_hostname(node.hostname)
    ...
    logger.info(
        "[node_placement] resolved placement=%s context=%s",
        [
            {
...
def select_free_nodes_from_allowed_ips(
    *,
    ...
    logger.info(
        f"[node_placement] selected allowlist nodes={node_ips} required_gpus={required} gpus_per_node={gpus_per_node}"
    )
