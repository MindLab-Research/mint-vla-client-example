...
ServiceClient 是访问 MinT 功能的主要接口。所有操作都从创建 ServiceClient 实例开始。
...
初始化用于 LoRA 微调的 TrainingClient。

**参数：**
- `base_model` - 模型标识符（例如 "Qwen/Qwen3-8B"）
- `rank` - LoRA 秩维度（默认：32）
- `train_mlp` - 训练 MLP 层（默认：True）
- `train_attn` - 训练注意力层（默认：True）
- `train_unembed` - 训练反嵌入层（默认：True）
- `seed` - 用于可重复性的随机种子
...
- **from_state** - 仅加载权重
- **from_state_with_optimizer** - 加载权重和优化器状态
...
访问用于检查点管理和元数据操作的 REST 端点。

## 异步变体

所有方法都包含异步变体（例如 `create_lora_training_client_async()`）。
