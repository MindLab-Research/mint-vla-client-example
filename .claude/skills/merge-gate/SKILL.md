...
VLA rows below assume a dedicated OpenPI VLA server is already running on the assigned worker, started from the dedicated vePFS root on `mint-dev` (for example via `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && bash scripts/wip/openpi_vla_start_server.sh <namespace> <vla-port> <log_file>'`).
...
| `vla_sft_pi0_fast_libero` | PR 422 validated task-16 SFT trace | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_libero_sft.py --base-url http://localhost:<vla-port> --base-model openpi/pi0-fast-libero-low-mem-finetune --task-index 16 --steps 12 --batch-size 4 --output-dir results/merge-gate/<ts>/vla_sft_pi0_fast_libero'` | Real OpenPI FAST supervised fine-tuning on LIBERO task 16 (`turn on the stove`) through the Mint VLA training surface, with a validated downward loss curve. |
| `vla_sft_pi05_libero` | PR 422 validated task-10 SFT trace | `openpi/pi05-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_libero_sft.py --base-url http://localhost:<vla-port> --base-model openpi/pi05-libero-low-mem-finetune --task-index 10 --steps 12 --batch-size 4 --output-dir results/merge-gate/<ts>/vla_sft_pi05_libero'` | Real OpenPI pi0.5 supervised fine-tuning on LIBERO task 10 (`put the bowl on the plate`) with a validated downward loss curve. |
| `vla_rl_pi0_fast_libero` | PR 422 validated short PPO trace | `openpi/pi0-fast-libero-low-mem-finetune` | 2 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_libero_fast_group_rl.py --base-url http://localhost:<vla-port> --task-index 16 --steps 6 --groups-per-step 4 --group-size 4 --learning-rate 1e-5 --temperature 0.05 --eval-temperature 0.0 --training-state-path mint://sft-cc060b0c47f6_0/sampler_weights/live-broad-83a61bef --train-item-indices 0,4,7,10,2,6,3 --eval-item-indices 1,5,8,9 --output-dir results/merge-gate/<ts>/vla_rl_pi0_fast_libero'` | Current representative pi0-fast PPO trace on the corrected VLA stack, including real action-session sampling, nonzero same-state advantages, and post-update PPO diagnostics. |
...
| `vla_resume_pi0_fast_libero` | PR 422 validated resume trace | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_libero_resume_sft.py --base-url http://localhost:<vla-port> --base-model openpi/pi0-fast-libero-low-mem-finetune --task-index 16 --pre-steps 4 --post-steps 4 --batch-size 4 --output-dir results/merge-gate/<ts>/vla_resume_pi0_fast_libero'` | Save-resume continuity for FAST LIBERO SFT on the shared OpenPI trainer, validated by a lower first resumed loss than the pre-save loss on task 16. |
| `vla_resume_pi05_libero` | PR 422 validated resume trace | `openpi/pi05-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_libero_resume_sft.py --base-url http://localhost:<vla-port> --base-model openpi/pi05-libero-low-mem-finetune --task-index 10 --pre-steps 4 --post-steps 4 --batch-size 4 --output-dir results/merge-gate/<ts>/vla_resume_pi05_libero'` | Save-resume continuity for pi0.5 LIBERO SFT on the shared OpenPI trainer, validated by a lower first resumed loss than the pre-save loss on task 10. |
...
| `vla_sampling_isolation_dual_fast` | PR 422 isolation regression probe | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_vla_dual_sampling_isolation_probe.py --base-url http://localhost:<vla-port> --base-model openpi/pi0-fast-libero-low-mem-finetune --valid-training-state-path mint://sft-18fd557a2446_0/weights/single-item-overfit-16-6 --invalid-model-path mint://sft-cc060b0c47f6_0/sampler_weights/midrun100probe-20260406b --task-index 16 --item-index 6 --temperature 0.0 --output-json results/merge-gate/<ts>/vla_sampling_isolation_dual_fast.json'` | Valid/invalid mixed-case isolation on the shared FAST sampling path without cross-session contamination. |
| `vla_sampling_switch_pi0_fast` | PR 422 shared-action actor validation | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_vla_action_session_churn_probe.py --base-url http://localhost:<vla-port> --base-model openpi/pi0-fast-libero-low-mem-finetune --model-paths mint://rl-5a5bf93cb35d_0/sampler_weights/group-rl-eval-baseline,mint://rl-5a5bf93cb35d_0/sampler_weights/group-rl-eval-3 --iterations 8 --task-index 16 --item-index 4 --temperature 0.0 > results/merge-gate/<ts>/vla_sampling_switch_pi0_fast.json'` | One shared FAST action actor alternates two tested checkpoints without deterministic drift on the validated pair. |
| `vla_pressure_shared_openpi` | PR 422 mixed-client pressure run | `mixed OpenPI` | 2 | `ssh mint-dev 'cd /vePFS-Mindverse/share/code/root/tinker-server-pr422-vla-20260402 && . ./configs/prod_volcano.env.sh >/dev/null 2>&1 && export PYTHONPATH=$PWD/scripts/wip:$PWD/openpi/src:$PWD && /vePFS-Mindverse/share/code/root/mint-runtime-py31213-openpi-pr422-20260402/host-venv/bin/python scripts/wip/openpi_vla_pressure_threads.py --base-url http://localhost:<vla-port> --output-root results/merge-gate/<ts>/vla_pressure_shared_openpi'` | Thirty logical VLA clients hit the shared OpenPI stack at once and record per-client success or `503/500` action-session creation failures instead of hiding them. |
