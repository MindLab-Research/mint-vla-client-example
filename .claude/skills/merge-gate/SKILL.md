...
| `vla_sampling_isolation_dual_fast` | current-server action-session smoke replacement | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_vla_smoke.py --base-url "${MINT_BASE_URL:-http://localhost:8000}" --api-key "${MINT_API_KEY:-dummy}" --model openpi/pi0-fast-libero-low-mem-finetune --output-json results/merge-gate/<ts>/vla_sampling_isolation_dual_fast.json` | Action-session sampling path on the active server with both train and act surfaces exercised. |
| `vla_sampling_switch_pi0_fast` | current-server repeat-session churn probe | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_vla_smoke.py --base-url "${MINT_BASE_URL:-http://localhost:8000}" --api-key "${MINT_API_KEY:-dummy}" --model openpi/pi0-fast-libero-low-mem-finetune --output-json results/merge-gate/<ts>/vla_sampling_switch_pi0_fast.run1.json && python scripts/wip/openpi_vla_smoke.py --base-url "${MINT_BASE_URL:-http://localhost:8000}" --api-key "${MINT_API_KEY:-dummy}" --model openpi/pi0-fast-libero-low-mem-finetune --output-json results/merge-gate/<ts>/vla_sampling_switch_pi0_fast.run2.json` | Repeated action-session creation and sampling on the same endpoint to detect churn regressions. |
| `vla_pressure_shared_openpi` | PR 422 mixed-client pressure run | `mixed OpenPI` | 2 | `python scripts/wip/openpi_vla_pressure_threads.py --base-url "${MINT_BASE_URL:-http://localhost:8000}" --output-root results/merge-gate/<ts>/vla_pressure_shared_openpi` | Concurrent VLA client pressure against the active server endpoint. |
