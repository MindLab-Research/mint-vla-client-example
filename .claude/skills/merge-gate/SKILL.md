...
| `vla_sft_pi0_fast_libero` | PR 422 VLA bringup | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_libero_sft.py --base-model openpi/pi0-fast-libero-low-mem-finetune --task-index 16 --steps 12 --batch-size 2 --output-dir results/merge-gate/<ts>/vla_sft_pi0_fast_libero` | Real OpenPI FAST supervised fine-tuning on LIBERO through the MintX VLA training surface and shared OpenPI trainer. |
| `vla_sft_pi05_libero` | PR 422 VLA bringup | `openpi/pi05-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_libero_sft.py --base-model openpi/pi05-libero-low-mem-finetune --task-index 10 --steps 12 --batch-size 2 --output-dir results/merge-gate/<ts>/vla_sft_pi05_libero` | Real OpenPI pi0.5 supervised fine-tuning on LIBERO through the MintX VLA training surface and shared OpenPI trainer. |
| `vla_rl_pi0_fast_libero` | PR 422 VLA bringup | `openpi/pi0-fast-libero-low-mem-finetune` | 2 | `python scripts/wip/openpi_libero_fast_rl.py --task-index 16 --steps 4 --batch-size 1 --output-dir results/merge-gate/<ts>/vla_rl_pi0_fast_libero` | Real FAST action-session sampling plus PPO-style update on LIBERO, including sampler export and Mint action-session flow. |
...
| `vla_resume_pi0_fast_libero` | PR 422 VLA bringup | `openpi/pi0-fast-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_libero_resume_sft.py --base-model openpi/pi0-fast-libero-low-mem-finetune --task-index 16 --pre-steps 4 --post-steps 4 --output-dir results/merge-gate/<ts>/vla_resume_pi0_fast_libero` | Save-resume continuity for FAST LIBERO SFT on the shared OpenPI trainer without a post-resume loss spike. |
| `vla_resume_pi05_libero` | PR 422 VLA bringup | `openpi/pi05-libero-low-mem-finetune` | 1 | `python scripts/wip/openpi_libero_resume_sft.py --base-model openpi/pi05-libero-low-mem-finetune --task-index 10 --pre-steps 4 --post-steps 4 --output-dir results/merge-gate/<ts>/vla_resume_pi05_libero` | Save-resume continuity for pi0.5 LIBERO SFT on the shared OpenPI trainer without a post-resume loss spike. |
...
| `vla_pressure_shared_openpi` | PR 422 VLA bringup | `mixed OpenPI` | 2 | `python scripts/wip/openpi_vla_pressure_threads.py --output-root results/merge-gate/<ts>/vla_pressure_shared_openpi` | Thirty concurrent logical VLA clients reuse the shared FAST and pi0.5 OpenPI trainers instead of duplicating trainer actors. |
