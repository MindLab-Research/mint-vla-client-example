"""MoE RL Test: Arithmetic with PPO.
...
class TestMoERL:
    """MoE model RL training tests."""

    def test_arithmetic_rl(self, moe_tokenizer):
        """Test MoE arithmetic RL training with importance sampling.

        Expected: Rewards improve over iterations, policy ratio stays near 1.0.
        Saves training curve for visual inspection.
        """
        num_iterations = 5  # Fewer iterations for MoE (slower)
        lr = 1e-4
        ...
        save_weights(model_id, name="moe_rl_initial")
        ...
        data_path, plot_path = save_training_curve(
            ...
            plot_title="MoE RL (Arithmetic): Training Curves"
        )
