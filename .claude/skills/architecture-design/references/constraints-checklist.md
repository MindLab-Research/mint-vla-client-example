...
- Adding a new endpoint
  ...
  - If work is async, return `request_id` and use `FutureStore` + `/retrieve_future` semantics.
...
- Adding or changing a Ray actor type
  ...
  - Register it in `tinker_server/backend/resource_pool.py` so GPU accounting and eviction stays correct.
