...
- Ray "GPU availability" is scheduling-level, not CUDA-memory-level. Mint relies on `ModelActorSupervisor` node-pin reconciliation, `ModelWorkScheduler` leases, `ModelActorSupervisorInventory` inventory, and placement groups (Megatron + multi-node vLLM) to keep large GPU reservations schedulable.
...
- Adding or changing a Ray actor type
  ...
  - If it is GPU-using, register live actors in `tinker_server/backend/model_actor_supervisor_inventory.py` so local inventory, inflight marking, and eviction safeguards stay correct.
