...
3. `MultiLoRAInferenceEngine.initialize()` connects to an existing detached vLLM actor or creates a new one:
  ...
  - registers the actor in `ModelActorSupervisorInventory` for LRU eviction and GPU accounting.
...
- `ModelActorSupervisorInventory` accounts for the full `total_required_gpus` so eviction decisions reflect the real cluster footprint.
