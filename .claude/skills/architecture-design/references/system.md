...
- API server: `mint_server/app.py` (FastAPI). Owns HTTP, auth, request validation, and lightweight in-process state.
...
- `mint_session_index_store`: minimal session and sampler metadata for REST enumeration after API restart.
- `mint_training_session_store`: minimal training-session metadata used to recover routing to detached trainer actors.
- `mint_gateway_session_store`: routing metadata for upstream-created sampling sessions and training models when this server acts as a gateway.
...
- Startup reconciliation is required to rediscover or kill these actors (`mint_server/app.py:_cleanup_stale_actors`).
