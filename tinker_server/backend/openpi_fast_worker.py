...
class OpenPIFastWorkerSession:
    ...
    def _load_train_state_checkpoint(self, path: Path) -> Any:
        ...
        if not resuming:
            ...
            if callable(close):
                close()
            raise FileNotFoundError(f"OpenPI FAST checkpoint has no saved steps: {checkpoint_path}")
        try:
    ...
    def _load_session_train_state_checkpoint(self, path: Path) -> Any:
        ...
        if not resuming:
            ...
            if callable(close):
                close()
            raise FileNotFoundError(f"OpenPI FAST session checkpoint has no saved steps: {path}")
        try:
        ...
        if ema_params is None and restored_ema_params is not None:
            raise ValueError("OpenPI FAST session checkpoint unexpectedly contains ema_params")
        if ema_params is not None and restored_ema_params is None:
            raise ValueError("OpenPI FAST session checkpoint is missing ema_params")
    ...
    def load_session_state(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        restored = self._session_state_manager.load_state(
            session_id,
            expected_worker_module="tinker_server.backend.openpi_fast_worker",
