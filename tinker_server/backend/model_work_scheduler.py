...
def _model_work_scheduler_debug_log_path() -> str:
    return os.environ.get(
        ...
        "/tmp/tinker_model_work_scheduler.debug.jsonl",
    )
...
class _ModelWorkSchedulerActor:
    ...
    def _expire_leases_locked(self, *, now: float) -> int:
        expired = 0
    ...
    def _assign_pending_locked(self, *, max_items: int | None = None) -> dict[str, Any]:
        assigned = 0
    ...
    async def append(
        ...
        *,
        ...
        async with self._cv:
            ...
            self._appended += 1
            assigned = (
                self._assign_pending_locked(max_items=assign_max_items)
                if bool(assign)
    ...
    async def sync_replicas(self, replicas: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        async with self._cv:
            ...
            expired = self._expire_leases_locked(now=now)
            assigned_pending = self._assign_pending_locked()
            self._cv.notify_all()
            return {
                "ok": True,
    ...
    async def assign_pending(self, *, max_items: int | None = None) -> dict[str, Any]:
        async with self._cv:
            expired = self._expire_leases_locked(now=time.time())
            out = self._assign_pending_locked(max_items=max_items)
            out["expired"] = expired
            self._cv.notify_all()
            return out
        ...
        *,
        ...
        async with self._cv:
            ...
            if requeue:
                ...
                self._failed += 1
            self._cv.notify_all()
            return {"ok": True, "request_id": lease.item.request_id, "requeued": bool(requeue)}

    async def expire_leases(self, *, now: float | None = None) -> dict[str, Any]:
        ts = time.time() if now is None else float(now)
        async with self._cv:
            expired = self._expire_leases_locked(now=ts)
            if expired:
                self._cv.notify_all()
