...
from .task_state_store import TaskStateNotFoundError, TaskStateStoreClient, task_state_store
...
class TaskStateFutureStore:
    """FutureStore-compatible facade backed by TaskStateStore.

    This keeps existing route/executor call sites working while moving the
    authority for futures to TaskStateStore and TaskPayloadStore.
    """
    ...
    @property
    def _payloads(self) -> TaskPayloadStore:
    ...
    async def async_fail(self, request_id: str, error: str) -> None:
        ...
        try:
            ...
            await self._task_state.async_ensure_task(
                request_id=str(request_id),
                status="failed",
                metadata={"error": str(error), "failed_at": time.time()},
            )
            await self._task_state.async_complete_task_failure(
                request_id=str(request_id),
                error=str(error),
...
class FutureStore:
    ...
    async def async_debug_snapshot(self, *, timeout_s: float = 10.0) -> dict[str, Any]:
        ...
        try:
            ...
            if not ray.is_initialized():
                out["ray_address_configured"] = preferred_ray_address()
                return out
...
future_store = TaskStateFutureStore()
