...
from mint_server.backend.training.verl.verl_inference import _extract_sampled_token_logprobs
...
def test_issue_355_extract_sampled_token_logprobs_logs_structure_for_missing_payload(caplog: pytest.LogCaptureFixture) -> None:
    with caplog.at_level(logging.WARNING, logger="mint_server.backend.training.verl.verl_inference"):
        with pytest.raises(
...
def test_issue_355_verl_requests_positive_sampled_token_logprob_budget() -> None:
    ...
    text = (repo_root / "mint_server/backend/training/verl/verl_inference.py").read_text(encoding="utf-8")
