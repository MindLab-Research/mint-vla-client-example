...
async def test_issue_364_training_route_restores_snapshot_from_detached_store(
    ...
    manager = TrainingSessionManager()
    engine = SimpleNamespace(_workers={}, _model_actor_registry_actor_names={})
...
async def test_issue_364_training_route_refreshes_step_without_version_bump(
    ...
    monkeypatch.setattr(training_route, "training_manager", manager)
    monkeypatch.setattr(training_route, "training_engine", SimpleNamespace(_workers={}, _model_actor_registry_actor_names={}))
...
def test_issue_364_refresh_training_session_drops_stale_worker_binding_when_actor_changes(
    ...
    engine = SimpleNamespace(
        ...
        _model_actor_registry_actor_names={"model-364-binding": "old-actor"},
    )
    ...
    assert engine._model_actor_registry_actor_names["model-364-binding"] == "new-actor"
...
async def test_issue_364_get_tokenizer_falls_back_to_local_metadata_when_worker_lookup_fails(
    ...
    monkeypatch.setattr(
        training_route,
        "training_engine",
        SimpleNamespace(_workers={}, _model_actor_registry_actor_names={}, get_tokenizer_info=_broken_get_tokenizer_info),
    )
