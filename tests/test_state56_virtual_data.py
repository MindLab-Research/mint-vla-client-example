...
def _store_and_row():
    ...
    store=State56SidecarStore.__new__(State56SidecarStore);store._positions={7:0};store._splits={7:'train'};store.dataset=_FakeDataset(row)
    return store,state
...
def test_state56_sidecar_split_lookup_is_exact():
    store,_=_store_and_row()
    assert store.split_for_source_row(7)=='train'
    with pytest.raises(ValueError,match='outside the Grade-A'):
        store.split_for_source_row(8)
