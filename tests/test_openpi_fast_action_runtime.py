...
def test_recover_detached_action_runtime_client_uses_shared_client_for_shared_actor(monkeypatch) -> None:
    ...
    class _FakePool:
        ...
        def get(self, actor_name):
            assert actor_name == "mint_openpi_shared_deadbeef"
            return None
    ...
    monkeypatch.setattr(action_session_manager, "get_model_actor_supervisor", lambda: _FakePool())
...
def test_start_openpi_action_ray_runtime_registers_actor_metadata_in_model_actor_inventory(monkeypatch) -> None:
    ...
    assert state["options"]["runtime_env"]["env_vars"]["MINT_OPENPI_FAST_ACTION_SESSION_STATE_ROOT"] == (
        f"/repo/checkpoints/openpi_action_session_state/mint/{expected_actor_name}"
    )
...
def test_start_openpi_action_ray_runtime_applies_single_node_pin(monkeypatch) -> None:
    ...
    assert options["runtime_env"]["env_vars"]["MINT_OPENPI_FAST_ACTION_SESSION_STATE_ROOT"] == (
        f"/repo/checkpoints/openpi_action_session_state/mint/{state['register']['actor_name']}"
    )
