...
def test_state41_layout_omits_fingertip_radial_rates():
    frames = 8
    ...
    result = state41.assemble_state41_sequence(
        hand_qpos=qpos,
        contacts=contacts,
        object_lift=lift,
    ...
    np.testing.assert_array_equal(result[:, 39], floor)
    np.testing.assert_allclose(result[1:4, 40], [0.0, 0.005, 0.010], atol=1e-7)


def test_action32_is_absolute_target28_plus_physical_zero_padding():
    targets = np.arange(4 * 28, dtype=np.float32).reshape(4, 28)
    actions = state41.absolute_target_actions32(targets)
    assert actions.shape == (4, 32)
    np.testing.assert_array_equal(actions[:, :28], targets)
    np.testing.assert_array_equal(actions[:, 28:], 0.0)
    assert state41.MANO_28D_DELTA_MASK_SEGMENTS == (3, -3, 22, -4)


def test_state41_timestamp_contract_is_exact_200hz():
    state41.validate_timestamps(np.arange(5) * 0.005, 5)
    with pytest.raises(ValueError, match="5 ms"):
        state41.validate_timestamps(np.asarray([0.0, 0.006]), 2)
...
def test_client_target_projection_accepts_target28_and_padding4():
    ...
    state = np.zeros((3, 41), dtype=np.float32)
